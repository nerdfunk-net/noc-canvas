...
class DeviceCache(Base):
    ...
    interfaces = relationship(
        "InterfaceCache", back_populates="device", cascade="all, delete-orphan"
    )
    ip_addresses = relationship(
        "IPAddressCache", back_populates="device", cascade="all, delete-orphan"
    )
    arp_entries = relationship(
        "ARPCache", back_populates="device", cascade="all, delete-orphan"
    )
    static_routes = relationship(
        "StaticRouteCache", back_populates="device", cascade="all, delete-orphan"
    )
    ospf_routes = relationship(
        "OSPFRouteCache", back_populates="device", cascade="all, delete-orphan"
    )
    bgp_routes = relationship(
        "BGPRouteCache", back_populates="device", cascade="all, delete-orphan"
    )
    mac_table_entries = relationship(
        "MACAddressTableCache", back_populates="device", cascade="all, delete-orphan"
    )
    cdp_neighbors = relationship(
        "CDPNeighborCache", back_populates="device", cascade="all, delete-orphan"
    )
...
class InterfaceCache(Base):
    ...
    device_id = Column(
        String, ForeignKey("device_cache.device_id", ondelete="CASCADE"), nullable=False
    )
    ...
    ip_addresses = relationship(
        "IPAddressCache", back_populates="interface", cascade="all, delete-orphan"
    )
...
class IPAddressCache(Base):
    ...
    )  # Indexed for "which device has this IP" queries
    ...
    __table_args__ = (Index("ix_ip_device_interface", "device_id", "interface_name"),)
...
class ARPCache(Base):
    ...
    # Indexes for common queries
    __table_args__ = (Index("ix_arp_device_ip", "device_id", "ip_address"),)
...
class StaticRouteCache(Base):
    ...
    # Indexes for common queries
    __table_args__ = (Index("ix_static_route_device_network", "device_id", "network"),)
...
class OSPFRouteCache(Base):
    ...
    # Indexes for common queries
    __table_args__ = (Index("ix_ospf_route_device_network", "device_id", "network"),)
...
class BGPRouteCache(Base):
    ...
    # Indexes for common queries
    __table_args__ = (Index("ix_bgp_route_device_network", "device_id", "network"),)
...
class JSONBlobCache(Base):
    ...
    )  # Command that was executed (e.g., "show interfaces")
    updated_at = Column(
        DateTime(timezone=True), server_default=func.now(), onupdate=func.now()
    )
...
class Snapshot(Base):
    ...
    device_name = Column(
        String, nullable=False, index=True
    )  # Device name for easier querying
    command = Column(
        String, nullable=False
