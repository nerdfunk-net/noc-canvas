...
class BaselineComparator:
    ...
    def compare_current_to_baseline(
        ...
        # Calculate differences
        differences = BaselineComparator._calculate_differences(
        ...
        for key in common:
            ...
            for field in all_fields:
                ...
                if old_value != new_value:
                    item_changes[field] = {"old": old_value, "new": new_value}

            if item_changes:
                changed.append({identifier_field: key, "changes": item_changes})
...
def format_comparison_report(comparison_result: Dict[str, Any]) -> str:
    ...
    # Header info
    report_lines.append(
        f"Device: {comparison_result.get('device_name', 'Unknown')} ({comparison_result.get('device_id')})"
    )
    ...
    if "baseline_old" in comparison_result:
        report_lines.append("Comparing:")
        report_lines.append(
            f"  Old: Version {comparison_result['baseline_old']['version']} - {comparison_result['baseline_old']['updated_at']}"
        )
        report_lines.append(
            f"  New: Version {comparison_result['baseline_new']['version']} - {comparison_result['baseline_new']['updated_at']}"
        )
    else:
        report_lines.append(
            f"Baseline: Version {comparison_result['baseline']['version']} - {comparison_result['baseline']['updated_at']}"
        )
    ...
    report_lines.append(
        f"  Changes Detected: {'YES' if comparison_result['has_changes'] else 'NO'}"
    )
