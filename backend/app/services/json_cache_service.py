...
class JSONCacheService:
    ...
    def set_cache(
        ...
        existing = db.query(JSONBlobCache).filter(
            and_(
        ...
        if existing:
            ...
            db.commit()
            db.refresh(existing)
            logger.info(f"Updated JSON cache for device {device_id}, command: {command}")
            return existing
        else:
            # Create new entry
            ...
            logger.info(f"Created JSON cache for device {device_id}, command: {command}")
            return cache_entry
    ...
    def get_cache(
        ...
        if command:
            ...
            logger.debug(f"Retrieved JSON cache for device {device_id}, command: {command}")
            return result
        else:
            results = query.all()
            logger.debug(f"Retrieved {len(results)} JSON cache entries for device {device_id}")
            return results
    ...
    def delete_cache(
        ...
        if command:
            logger.info(f"Deleted JSON cache for device {device_id}, command: {command} ({count} entries)")
        else:
            logger.info(f"Deleted all JSON cache entries for device {device_id} ({count} entries)")
    ...
    def get_cached_commands(db: Session, device_id: str) -> List[str]:
        ...
        """
        results = db.query(JSONBlobCache.command).filter(
            JSONBlobCache.device_id == device_id
        ).all()
        commands = [r[0] for r in results]
        logger.debug(f"Retrieved {len(commands)} cached commands for device {device_id}")
        return commands
    ...
    def is_cache_valid(db: Session, cache_entry: JSONBlobCache) -> bool:
        ...
        logger.debug(f"Cache validity check: updated_at={cache_entry.updated_at}, expiration={expiration_time}, now={now}, valid={is_valid}")
        return is_valid
